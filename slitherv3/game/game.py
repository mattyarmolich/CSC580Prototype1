...
class Game:
    ...
    def __init__(self, game_model, fps, pixel_size, screen_width, screen_height, navigation_bar_height):
        ...
        self.wall.points = list([self._screen_normalized_point(x) for x in self.environment.set_wall()])
        self.screen_objects.append(self.wall)

        self.fruit = FruitScreenObject(self)
        self.fruit.points = list([self._screen_normalized_point(x) for x in self.environment.set_fruit()])
        self.screen_objects.append(self.fruit)

        self.snake = SnakeScreenObject(self)
        self.snake.points = list([self._screen_normalized_point(x) for x in self.environment.set_snake()])
    ...
    def _draw_screen(self):
        ...
        for game_object in self.screen_objects:
            ...
            # changed to int(...)  instead of without int cast
    ...
    def _sync_screen_with_environment(self):
        self.fruit.points = list([self._screen_normalized_point(x) for x in self.environment.fruit])
        self.snake.points = list([self._screen_normalized_point(x) for x in self.environment.snake])
