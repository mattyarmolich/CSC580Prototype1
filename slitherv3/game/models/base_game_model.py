...
class BaseGameModel:
    ...
    def _best_action_for_runs(self, runs):
        scores_for_actions = {}
        for run in runs:
            if run.action in list(scores_for_actions.keys()):
        ...
        for action in list(scores_for_actions.keys()):
        ...
        sorted_average_scores_for_actions = OrderedDict(sorted(list(average_scores_for_actions.items()), key=lambda t: t[1]))
